...
const ChatbotRequestReplyInterface = () => {
  ...
  const call_server = (input, mode, command) => {
    console.log('input= %s, mode=%s, command=%s', input, mode, command)
    const data = {text: input, type: mode, command: command};
  ...
  const handleSubmit = (event) => {
    ...
    if (result) {
      ...
      call_server(input, mode, 'get-response');
  };


  const handleClearHistory = () => {
    setMessages([]);
    call_server('clear', mode, 'clear-history');
  }
